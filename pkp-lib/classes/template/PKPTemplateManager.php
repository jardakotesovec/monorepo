...
require_once('./' . PKP_LIB_PATH . '/lib/vendor/smarty/smarty/libs/plugins/modifier.escape.php'); // Seems to be needed?

/* This definition is required by Smarty */
define('SMARTY_DIR', Core::getBaseDir() . '/' . PKP_LIB_PATH . '/lib/vendor/smarty/smarty/libs/');
...
class PKPTemplateManager extends Smarty
{
    ...
    /**
     * Get URL for static files in the app directory.
     * In monorepo mode, prefixes with app name: e.g., '/ojs/js/build.js'
     * In standalone mode, returns the path as-is.
     */
    protected function getAppFileUrl(string $baseUrl, string $path): string
    {
        if (defined('PKP_APPLICATION')) {
            return $baseUrl . '/' . PKP_APPLICATION . '/' . ltrim($path, '/');
        }
    ...
    */
    ...
    public function __construct()
    {
        ...
        $coreTemplateDir = PKP_LIB_PATH . '/templates';
    ...
    public function registerJSLibrary(): void
    {
        ...
        $this->addJavaScript(
            'jqueryValidate',
            $this->getAppFileUrl($baseUrl, 'js/build/jquery-validation/jquery.validate.min.js'),
            $args
        );
        ...
        foreach ($localeChecks as $localeCheck) {
            if (file_exists($jqvLocalePath . $localeCheck . '.js')) {
                $this->addJavaScript('jqueryValidateLocale', $this->getAppFileUrl($baseUrl, $jqvLocalePath . $localeCheck . '.js'), $args);
            }
        }

        $this->addJavaScript(
            'plUpload',
            $baseUrl . '/' . PKP_LIB_URL_PATH . '/lib/vendor/moxiecode/plupload/js/plupload.full.min.js',
            $args
        );
        $this->addJavaScript(
            'jQueryPlUpload',
            $baseUrl . '/' . PKP_LIB_URL_PATH . '/lib/vendor/moxiecode/plupload/js/jquery.ui.plupload/jquery.ui.plupload.js',
            $args
        );
        $plLocalePath = PKP_LIB_PATH . '/lib/vendor/moxiecode/plupload/js/i18n/';
        foreach ($localeChecks as $localeCheck) {
            if (file_exists($plLocalePath . $localeCheck . '.js')) {
                $this->addJavaScript('plUploadLocale', $baseUrl . '/' . PKP_LIB_URL_PATH . '/lib/vendor/moxiecode/plupload/js/i18n/' . $localeCheck . '.js', $args);
            }
        }
        ...
        $this->addJavaScript(
            'pkpApp',
            $this->getAppFileUrl($baseUrl, 'js/build.js'),
            [
                'priority' => self::STYLE_SEQUENCE_LATE,
                'contexts' => ['backend']
            ]
        ...
        if (Config::getVar('general', 'enable_minified')) {
            $this->addJavaScript(
                'pkpLib',
                $this->getAppFileUrl($baseUrl, 'js/pkp.min.js'),
                [
                    'priority' => self::STYLE_SEQUENCE_CORE,
                    'contexts' => ['backend']
                ]
        ...
        foreach ($minifiedScripts as $key => $script) {
            if (str_starts_with($script, 'lib/pkp/')) {
                $scriptUrl = $baseUrl . '/' . PKP_LIB_URL_PATH . substr($script, 7);
            } else {
                $scriptUrl = $this->getAppFileUrl($baseUrl, $script);
            }
            $this->addJavaScript('pkpLib' . $key, $scriptUrl, $args);
        }
    }
    ...
    public function registerJSLibraryData(): void
    {
        ...
        $app_data = [
            ...
            'tinyMceContentCSS' => [$this->getAppFileUrl($this->_request->getBaseUrl(), 'plugins/generic/tinymce/styles/content.css'), $this->_request->getBaseUrl() . '/' . PKP_LIB_URL_PATH . '/styles/mailables/style.css'],
            'tinyMceOneLineContentCSS' => $this->getAppFileUrl($this->_request->getBaseUrl(), 'plugins/generic/tinymce/styles/content_oneline.css'),
        ];
    ...
    public function requiresVueRuntime()
    {
        if (!$this->isVueRuntimeIncluded) {
            ...
            $this->addStyleSheet(
                'build',
                $this->getAppFileUrl($baseUrl, 'styles/build_frontend.css'),
                [
                    'priority' => self::STYLE_SEQUENCE_CORE,
                    'contexts' => ['frontend'],
                ]
            ...
            $this->addJavaScript(
                'pkpAppFrontend',
                $this->getAppFileUrl($baseUrl, 'js/build_frontend.js'),
                [
                    'priority' => self::STYLE_SEQUENCE_CORE,
                    'contexts' => ['frontend']
                ]
    ...
    public function setupBackendPage()
    {
        ...
        $this->addJavaScript(
            'jquery',
            $this->getAppFileUrl($request->getBaseUrl(), 'js/build/jquery/jquery' . $min . '.js'),
            [
                'priority' => self::STYLE_SEQUENCE_CORE,
                'contexts' => 'backend',
            ]
        ...
        $this->addJavaScript(
            'jqueryUI',
            $this->getAppFileUrl($request->getBaseUrl(), 'js/build/jquery-ui/jquery-ui' . $min . '.js'),
            [
                'priority' => self::STYLE_SEQUENCE_CORE,
                'contexts' => 'backend',
            ]
        ...
        $this->addStyleSheet(
            'fontAwesome',
            $request->getBaseUrl() . '/' . PKP_LIB_URL_PATH . '/styles/fontawesome/fontawesome.css',
            [
                'priority' => self::STYLE_SEQUENCE_CORE,
                'contexts' => 'backend',
            ]
        ...
        $this->addStyleSheet(
            'build',
            $this->getAppFileUrl($request->getBaseUrl(), 'styles/build.css'),
            [
                'priority' => self::STYLE_SEQUENCE_CORE,
                'contexts' => 'backend',
            ]
    ...
    {
        return $this->getAppFileUrl($request->getBaseUrl(), PKP_LIB_PATH . '/styles/tinymce');
    }
    ...
    public function smartyHtmlOptionsTranslate($params, $smarty)
    {
        ...
        require_once(PKP_LIB_PATH . '/lib/vendor/smarty/smarty/libs/plugins/function.html_options.php');
