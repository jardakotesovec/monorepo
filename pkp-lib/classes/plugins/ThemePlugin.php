...
abstract class ThemePlugin extends LazyLoadPlugin
{
    ...
    public function _getBaseUrl($path = '')
    {
        ...
        $appPrefix = defined('PKP_APPLICATION') ? '/' . PKP_APPLICATION : '';
        return "{$request->getBaseUrl()}{$appPrefix}/{$this->getPluginPath()}$path";
    }
    ...
    protected function loadChartJavascript(): void
    {
        ...
        $templateMgr->addJavaScript(
            'usageStatsFrontend',
            $request->getBaseUrl() . '/' . PKP_LIB_URL_PATH . '/js/usage-stats-chart.js',
            [
                'contexts' => $this->getSubmissionViewContext(),
            ]
        );
